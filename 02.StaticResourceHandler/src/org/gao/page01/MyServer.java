 ...
 * @Description: 演示请求行解析，按 URL 返回对应资源，文件不存在时返回 404 页面
 * @Version: 1.0
 */
...
//        System.out.println(new String(bytes,0,len)); // requestMessage
        ...
        // 获取请求行 第一行 URL
        String requestLine = splitRequestMsg[0];

        // 按照空格切割请求行
        String[] splitRequestLine = requestLine.split(" ");


        System.out.println(Arrays.toString(splitRequestLine));

        System.out.println(splitRequestLine[1]);

        // 拿到URL
        String requestURL = splitRequestLine[1];
...
//        System.out.println(Arrays.toString(splitRequestMsg));
